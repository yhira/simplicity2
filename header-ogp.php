...
if (is_singular()){//単一記事ページの場合
  if(have_posts()): while(have_posts()): the_post();
    echo '<meta property="og:description" content="'.esc_attr(get_the_description()).'">';echo "\n";//抜粋を表示
  endwhile; endif;
  $title = get_the_title();
  if ( is_front_page() ) {
...
if (is_singular()){//単一記事ページの場合
  if (has_post_thumbnail()){//投稿にサムネイルがある場合の処理
    ...
    echo '<meta property="og:image" content="'.esc_url($image[0]).'">';echo "\n";
  } else if ( preg_match( $searchPattern, $content, $imgurl ) && !is_archive()) {//投稿にサムネイルは無いが画像がある場合の処理
    echo '<meta property="og:image" content="'.esc_url($imgurl[2]).'">';echo "\n";
  } else if ( get_ogp_home_image() ){//ホームイメージが設定されている場合
    echo '<meta property="og:image" content="'.esc_url(get_ogp_home_image()).'">';echo "\n";
...
<meta property="fb:app_id" content="<?php echo esc_attr(get_fb_app_id()); ?>">
<?php endif; ?>
<!-- /OGP -->
